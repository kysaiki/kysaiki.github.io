// js/projects-data.js
// ========== PROJECT DATA ==========

export const allProjects = [
  {
    id: "gearbox-tools",
    title: "Gearbox Tools & Pipeline",
    short: "Internal Unreal tooling and production pipelines at Gearbox.",
    detail: [
      "Contributed to internal Gearbox tooling focused on Unreal Engine editor extensions, data workflows, and studio-specific pipelines.",
      "Built and extended Slate-based tools that streamlined asset authoring, validation, and iteration for designers and artists.",
      "Collaborated with engineers and content teams to improve reliability, debuggability, and UX of editor-facing systems."
    ],
    variant: "alt",
    skills: ["Gearbox", "Unreal Engine", "Slate", "Tools", "Pipelines"],
    bgGif: "media/bg-gbx.jpg",
    overlayTemplateId: "overlay-gearbox-tools"
  },
  {
    id: "nba2k26",
    title: "NBA 2K26 — Engineering",
    short: "Production tools & game systems for NBA 2K26.",
    detail: [
      "Contributed to NBA 2K26 engineering, focusing on tools, workflows, and systems that support large-scale sports development.",
      "Built and refined pipelines that improve iteration speed for designers, artists, and gameplay engineers."
    ],
    variant: "primary",
    skills: ["2K", "Tools", "Production", "C++"],
    bgGif: "media/bg-nba-2k26.jpg",
    overlayTemplateId: "overlay-nba2k26"
  },
  {
    id: "egregore",
    title: "Egregore — Gameplay/Tools",
    short: "Gameplay systems and tools support for Egregore.",
    detail: [
      "Prototyped and implemented gameplay systems with a focus on readability, tuning, and iteration speed.",
      "Built supporting tools and workflows to help designers quickly configure encounters and content."
    ],
    variant: "primary",
    skills: ["Gameplay", "Unreal Engine", "Tools"],
    bgGif: "media/bg-egregore2.png",
    overlayTemplateId: "overlay-egregore"
  },
  {
    id: "couchwizards",
    title: "Couch Wizards — Solo Dev",
    short: "Local multiplayer party arena game, built solo.",
    detail: [
      "Designed and developed a local multiplayer experience with tight, responsive controls and rapid rounds.",
      "Handled everything from gameplay code and UI to build pipelines and polishing the feel of moment-to-moment play."
    ],
    variant: "primary",
    skills: ["Gameplay", "Unreal Engine", "Solo Dev"],
    bgGif: "media/bg-egregore2.png",
    overlayTemplateId: "overlay-couchwizards"
  }
];
